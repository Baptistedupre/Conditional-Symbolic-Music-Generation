...
def split_process_and_combine(
    note_sequence, split, sample_size, randomize, to_tensors_fn
):
    ...
    """
    note_sequences = (
        sequences_lib.split_note_sequence_on_time_changes(note_sequence)
        if split
        else [note_sequence]
    )
    results = []
...
def combine_converter_tensors(
    converter_tensors, max_num_tensors=None, randomize_sample=True
):
    ...
    for instrument in instruments:
        ...
        while 1:
            ...
            instrument_search_start_step = (
                melody.end_step + (search_start_step - melody.end_step) % steps_per_bar
            )
            if not melody:
                break
...
class NoteSequenceAugmenter(object):
    ...
    def tf_augment(self, note_sequence_scalar):
        ...
        augmented_note_sequence_scalar = torch.jit.script(_augment_str)(
...
class LegacyEventListOneHotConverter(BaseNoteSequenceConverter):
    ...
    def _dedupe_and_sample(self, all_sliced_lists):
        ...
        if self._dedupe_event_lists:
            multievent_tuples = list(set(tuple(l) for l in sliced_multievent_lists))
        else:
            multievent_tuples = [tuple(l) for l in sliced_multievent_lists]
        multievent_tuples = maybe_sample_items(
            multievent_tuples, self.max_tensors_per_notesequence, self.is_training
        )
        ...
        for ct, kt in itertools.zip_longest(chord_tuples, key_tuples):
            ...
            if ct is not None:
                ...
                controls.append(
                    np_onehot(
                        chord_tokens,
                        self._chord_encoding.num_classes,
                        self.control_dtype,
                    )
                )
    ...
    def from_tensors(self, samples, controls=None):
        ...
        for i, sample in enumerate(samples):
            ...
            if self._chord_encoding and controls is not None:
                chords = [
                    self._chord_encoding.decode_event(e)
...
class OneHotMelodyConverter(LegacyEventListOneHotConverter):
    ...
    def __init__(
        ...
        self._melody_encoding = note_seq.MelodyOneHotEncoding(min_pitch, max_pitch + 1)
    ...
    def _to_tensors_fn(self, note_sequence):
        def is_valid(note):
            if (
                self._valid_programs is not None
                and note.program not in self._valid_programs
            ):
                return False
    ...
    def to_tensors(self, item):
        note_sequence = item
        return split_process_and_combine(
            note_sequence,
