...
class MIDIDataset(Dataset):
    ...
    def __init__(self, data_dir: str = 'data/songs/', feature_type: str = 'OneHotGenre'):
        """
        Args:
    ...
    def __len__(self):
        return len(self.file_list) * 100  # Arbitrary multiplier to allow multiple samples per file
...
def convert_midi_to_tensors(midi_path: str, chunk_size: int = 2) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def preprocess_and_save_midi(midi_path: str, row: pd.Series, save_dir: str = 'data/songs/'):
    ...
    try:
        ...
        data_dict['OneHotGenre'] = torch.tensor(row['OneHotGenre']).to(torch.float8_e4m3fn)
